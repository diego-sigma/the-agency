...
- **Lance** — new PR-specialist persona (`templates/personas/lance.md`) and matching `/lance` skill. Drafts pull request descriptions matching the target repo's house style and pre-flights the diff for missed best practices. Backed by per-repo profiles in `templates/pr-author/<repo>.md` derived from analyzing the 500 most recently merged PRs (descriptions) + top 100 by review activity (best-practice critiques) per repo. Initial profiles ship for `sigmacomputing/slate` and `sigmacomputing/mono-node`, each with a sibling `<repo>.analyzed.json` manifest recording exactly which PR numbers were sampled so refreshes can be incremental. New routing-table row in `CLAUDE.md` directs "drafting a PR / pre-flighting a diff" tasks to Lance.
