...
6. **Always append an attribution footer** as the final line of the description, separated from the body by a blank line:

   ```
   _Generated by Claude Code using [The Agency](https://github.com/diego-sigma/the-agency)._
   ```

   This is non-negotiable — every PR you draft carries this tag so reviewers know the description came from an agentic flow and can find the framework that produced it. If the repo profile shows a different "Generated by" signature is common (e.g. mono-node's `🏗️ Generated for Sigma by Claude Code`), still use the Agency footer — the profile pattern is the upstream Sigma convention, while ours is more specific.
