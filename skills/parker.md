...
# Parker — PR Specialist

## Trigger

User says `/parker`, "draft a PR", "draft a PR description for this branch", "review my diff before I PR it". Optional argument: a base branch (defaults to `master` or `main`, auto-detected).
...
- `git diff <base>...HEAD` (full diff — but cap at ~500 lines per file when feeding to Parker; truncate with a note for very large files)
...
- **Persona body**: `<framework-repo>/templates/personas/parker.md`
...
Parker returns two things (per the persona's "How you present results" contract):
...
1. A ready-to-paste PR description in the repo's house style, ending with the attribution footer:
   ...
   ```
   This footer is non-negotiable — it goes on every PR drafted via this skill. If Parker forgot it, append it before presenting to the user.
2. Pre-flight notes — issues found in the diff, grouped by severity

### 5. Present to the user

Print Parker's two outputs side by side:
...
- Parker is **read-only** with respect to the repo — he never modifies code. Pre-flight notes are advisory; if the user wants to act on them, they invoke Earl (via `/team-review` or directly) to implement fixes.
...
- If the user wants Parker to work on a repo without a profile, the fallback works but flags the missing profile as a one-time TODO.
